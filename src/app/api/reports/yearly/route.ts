...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(await buildReport(start, end, "yearly"));
  } catch (err: any) { if (err instanceof NextResponse) return err; return NextResponse.json({ error: "Failed to generate yearly report" }, { status: 500 }); }
}

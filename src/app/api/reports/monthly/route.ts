...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(await buildReport(start, end, "monthly"));
  } catch (err: any) { if (err instanceof NextResponse) return err; return NextResponse.json({ error: "Failed to generate monthly report" }, { status: 500 }); }
}
